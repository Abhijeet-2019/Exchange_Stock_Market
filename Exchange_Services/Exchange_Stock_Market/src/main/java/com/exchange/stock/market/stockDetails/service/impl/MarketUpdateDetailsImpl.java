...
public class MarketUpdateDetailsImpl implements MarketUpdateDetails {
    ...
    /**
     * This method is used to fetch the Stock details for a date provided.
     * @param urlWithDate : Date
     * @throws IOException          :Throws IO Exception when file is not available.
     * @throws InterruptedException : Throws Interrupted Exception for network failure
     */
    private void populateDataFromURL(String urlWithDate) throws IOException, InterruptedException {
        ...
        String line;
        List<MarketStockInfo> stockList = new ArrayList<>();
        while ((line = reader.readLine()) != null) {
            ...
            MarketStockInfo stockDetails = getStockDetails(cols);
    ...
    private static MarketStockInfo getStockDetails(String[] cols) {
        MarketStockInfo stockDetails = new MarketStockInfo();
        stockDetails.setLastTradedDate(cols[0].trim());
        stockDetails.setBizDate(cols[0].trim());
